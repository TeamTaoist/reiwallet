...
export default function Grant(){
    ...
    const closeWin = async() =>{
        await messenger.send('grant_result', {status:"rejected",data:"user rejected"});
        window.close();
    }
    const handleGrant = async() =>{
        ...
        try{
            ...
            await messenger.send('grant_result', {status:"success",data:{result_type:"grant_success",grant_address:address}});
            window.close();
        }catch (e) {
            console.error('grant_result',e)
            await messenger.send('grant_result', {status:"failed",data:e.message});
        }finally {
            setLoading(false)
        }
    ...
    return  <Box>
        {
            showAccount && <AccountSwitch currentAccount={currentAccount} handleCurrent={handleCurrent}  />
        }
