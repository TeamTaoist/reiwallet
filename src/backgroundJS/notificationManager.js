...
class NotificationManager {
  ...
  async createNotificationWindow(_notification, options) {
    ...
    if (
      ...
      (this.isCurrentNotification(notification) ||
    ...
    browser.windows.onRemoved.addListener((windowId) => {
      if (windowId === window.id) {
        ...
        this.eventEmitter.emit("finish", {
          nextSessionId: this.notificationInfoQueue[0]?.sessionId,
        });
      }
    });
  ...
  getNotificationCount() {
    return this.currentNotification
      ? 1 + this.notificationInfoQueue.length
      : this.notificationInfoQueue.length;
  }
  async updateBadge() {
    let label = "";
  ...
  isCurrentNotification(notification) {
    return (
      ...
      isEqual(
        omit(notification, "sessionId"),
        omit(this.currentNotification, "sessionId"),
      )
    );
  }
