...
import {blockchain} from "@ckb-lumos/base";
  ...
  export const transfer_udt = async(options,network,currentAccount) => {
    const unsigned = await sudt_xudt_buildTransfer(options,network,currentAccount);
    return unsigned;
  }
...
const sudt_xudt_buildTransfer = async (options, network) => {
  ...
  if (change_amount.gt(0)) {
    ...
    txSkeleton = txSkeleton.update("outputs", (outputs) => outputs.push(outputs_sudt, outputs_sudt_change));
  } else {
